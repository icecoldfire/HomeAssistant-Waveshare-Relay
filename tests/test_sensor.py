...
def test_waveshare_relay_timer_device_info() -> None:
    """Obsolete: device_info should be checked after async_added_to_hass. Remove this test."""
    pass
...
async def test_switch_state_changed_on(mock_hass: MagicMock) -> None:
    ...
    with (
        patch.object(WaveshareRelayTimer, "async_write_ha_state") as mock_write_ha_state,
...
async def test_switch_state_changed_off(mock_hass: MagicMock) -> None:
    ...
    with patch.object(WaveshareRelayTimer, "async_write_ha_state") as mock_write_ha_state:
        timer = WaveshareRelayTimer(mock_hass, "192.168.1.100", 502, "Test Relay", 0)
        timer.entity_id = "sensor.test_timer"
...
async def test_countdown_timer(mock_hass: MagicMock) -> None:
    ...
    with patch.object(WaveshareRelayTimer, "async_write_ha_state") as mock_write_ha_state, patch("asyncio.sleep", new=AsyncMock()):
        timer = WaveshareRelayTimer(mock_hass, "192.168.1.100", 502, "Test Relay", 0)
        timer.entity_id = "sensor.test_timer"
...
async def test_invalid_interval_error_logging(mock_hass: MagicMock) -> None:
    ...
    with (
        ...
        patch.object(timer, "async_write_ha_state"),
    ):
        event = MagicMock(data={"new_state": MagicMock(state="on")})
        await timer._switch_state_changed(event)
...
def test_init_logs_error_when_switch_entity_not_found(mock_hass: MagicMock):
...
def test_switch_state_changed_interval_state_none(mock_hass: MagicMock):
    """Test _switch_state_changed defaults to 5 if interval_state is None."""
    with patch.object(WaveshareRelayTimer, "async_write_ha_state"):
        timer = WaveshareRelayTimer(mock_hass, "192.168.1.100", 502, "Test Relay", 0)
        timer.entity_id = "sensor.test_timer"
        with (
            patch("homeassistant.helpers.entity_registry.async_get", return_value=MagicMock(async_get_entity_id=lambda *args: timer.entity_id)),
...
async def test_device_info_after_async_added_to_hass():
    hass = MagicMock()
    with (
        patch("custom_components.waveshare_relay.sensor._read_device_address", new_callable=AsyncMock, return_value=1),
        patch("custom_components.waveshare_relay.sensor._read_software_version", new_callable=AsyncMock, return_value="1.0"),
    ):
        timer = WaveshareRelayTimer(hass, "192.168.1.100", 502, "Test Relay", 0)
...
async def test__async_load_device_info_handles_exceptions():
    hass = MagicMock()
    with (
        patch("custom_components.waveshare_relay.sensor._read_device_address", new_callable=AsyncMock, side_effect=Exception("fail")),
        patch("custom_components.waveshare_relay.sensor._read_software_version", new_callable=AsyncMock, side_effect=Exception("fail")),
    ):
        timer = WaveshareRelayTimer(hass, "192.168.1.100", 502, "Test Relay", 0)
        await timer._async_load_device_info()
        assert timer._device_address is None
        assert timer._sw_version is None


def test_all_properties():
    hass = MagicMock()
    timer = WaveshareRelayTimer(hass, "ip", 1, "name", 2)
    assert timer.unique_id == "waveshare_relay_ip_2_timer"
    assert timer.name == "3 Timer"
    assert timer.native_unit_of_measurement == "s"
