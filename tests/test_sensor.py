...
def test_waveshare_relay_timer_device_info() -> None:
    ...
    with (
        patch(
            "custom_components.waveshare_relay.sensor._read_device_address", return_value=1),
        patch("custom_components.waveshare_relay.sensor._read_software_version",
              return_value="1.0"),
    ):
        timer = WaveshareRelayTimer(
            hass, "192.168.1.100", 502, "Test Relay", 0)
...
async def test_switch_state_changed_on(mock_hass: MagicMock) -> None:
    ...
    with (
        ...
        patch.object(mock_hass.states, "get",
                     return_value=MagicMock(state="10")),
    ):
        timer = WaveshareRelayTimer(
            mock_hass, "192.168.1.100", 502, "Test Relay", 0)
...
async def test_invalid_interval_error_logging(mock_hass: MagicMock) -> None:
    ...
    with (
        patch("homeassistant.helpers.entity_registry.async_get", return_value=MagicMock(
            async_get_entity_id=lambda *args: timer.entity_id)),
        patch.object(mock_hass.states, "get",
        ...
        mock_logger.assert_called_with(
            "Invalid interval value for %s: %s", timer.entity_id, "invalid")
...
async def test_interval_entity_not_found_error(mock_hass: MagicMock) -> None:
    ...
    with (
        patch("homeassistant.helpers.entity_registry.async_get",
              return_value=MagicMock(async_get_entity_id=lambda *args: None)),
        patch("custom_components.waveshare_relay.sensor._LOGGER.error") as mock_logger,
        patch.object(timer, "async_write_ha_state", new=AsyncMock()),
    ):
        event = MagicMock(data={"new_state": MagicMock(state="on")})
        await timer._switch_state_changed(event)
        mock_logger.assert_called_with(
            "Could not find entity with unique_id: %s", "waveshare_relay_192.168.1.100_0_interval")
...
def test_init_logs_error_when_switch_entity_not_found(mock_hass: MagicMock):
    ...
    with patch("homeassistant.helpers.entity_registry.async_get", return_value=MagicMock(async_get_entity_id=lambda *args: None)), \
            patch("custom_components.waveshare_relay.sensor._LOGGER.error") as mock_logger:
        WaveshareRelayTimer(mock_hass, "192.168.1.100", 502, "Test Relay", 0)
        mock_logger.assert_called_with(
            "Could not find entity with unique_id: %s", "waveshare_relay_192.168.1.100_0_switch")
...
def test_switch_state_changed_interval_state_none(mock_hass: MagicMock):
    ...
    with patch.object(WaveshareRelayTimer, "async_write_ha_state", new_callable=AsyncMock):
        timer = WaveshareRelayTimer(
            mock_hass, "192.168.1.100", 502, "Test Relay", 0)
        timer.entity_id = "sensor.test_timer"
        with patch("homeassistant.helpers.entity_registry.async_get", return_value=MagicMock(async_get_entity_id=lambda *args: timer.entity_id)), \
