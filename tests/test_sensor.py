...
async def test_async_setup_entry(mock_hass, mock_config_entry):
    ...
    assert len(async_add_entities.call_args[0]
               [0]) == mock_config_entry.data["channels"]
...
def test_waveshare_relay_timer_device_info():
    ...
    hass = MagicMock()
    with patch("custom_components.waveshare_relay.sensor._read_device_address", return_value=1), \
